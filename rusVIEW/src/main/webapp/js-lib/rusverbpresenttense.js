	...
	remove: function(contextDoc) {
		...
		$('body').undelegate('span.wertiviewtoken', 'click', wertiview.rusverbpresenttense.clickHandler);
		$('body').undelegate('select.wertiviewinput', 'change', wertiview.rusverbpresenttense.clozeInputHandler);
		$('body').undelegate('span.wertiviewhint', 'click', wertiview.rusverbpresenttense.clozeHintHandler);
		$('body').undelegate('input.wertiviewinput', 'change', wertiview.rusverbpresenttense.clozeInputHandler);
		$('body').undelegate('input.wertiviewhint', 'click', wertiview.rusverbpresenttense.clozeHintHandler);  // was: span.wertiviewhint
...
span.find('span.wertiviewhit').addClass('colorizeStyleRusVerbPresentTense');
	...
	click: function(contextDoc) {
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.rusverbpresenttense.clickHandler); 
	},
	...
	mcGetOptions: function($hit, capType){
        ...
        while (j < wertiview.rusverbpresenttense.types.length && options.length < wertiview.rusverbpresenttense.MAX_MC - 1) {
            ...
            if (wertiview.rusverbpresenttense.types[j].toLowerCase() != $hit.text().toLowerCase() && wertiview.rusverbpresenttense.types[j] != "") 
            {
