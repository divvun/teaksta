	...
	remove: function(contextDoc) {
		...
		$('body').undelegate('span.wertiviewtoken', 'click', wertiview.rusnounsingular.clickHandler);
		$('body').undelegate('select.wertiviewinput', 'change', wertiview.rusnounsingular.clozeInputHandler);
		$('body').undelegate('span.wertiviewhint', 'click', wertiview.rusnounsingular.clozeHintHandler);
		$('body').undelegate('input.wertiviewinput', 'change', wertiview.rusnounsingular.clozeInputHandler);
		$('body').undelegate('input.wertiviewhint', 'click', wertiview.rusnounsingular.clozeHintHandler);  // was: span.wertiviewhint
...
span.find('span.wertiviewhit').addClass('colorizeStyleRusNounSingular');
	...
	click: function(contextDoc) {
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.rusnounsingular.clickHandler); 
	},
	...
	mcGetOptions: function($hit, capType){
        ...
        while (j < wertiview.rusnounsingular.types.length && options.length < wertiview.rusnounsingular.MAX_MC - 1) {
            ...
            if (wertiview.rusnounsingular.types[j].toLowerCase() != $hit.text().toLowerCase() && wertiview.rusnounsingular.types[j] != "") 
            {
	...
	clozeInputHandler: function(event) {
		...
		if($(this).val().toLowerCase() == $(this).data('wertiviewanswer').toLowerCase()) {
			...
			$("#"+$(this).parent().attr('clueid')).css( "color", "inherit" );
			$text = $("<span>");
			...
			$("#"+$(this).parent().attr('clueid')).css( "color", "red" );
