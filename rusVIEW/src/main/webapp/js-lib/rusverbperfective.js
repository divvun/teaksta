	...
	remove: function(contextDoc) {
		...
		$('body').undelegate('span.wertiviewtoken', 'click', wertiview.rusverbperfective.clickHandler);
		$('body').undelegate('select.wertiviewinput', 'change', wertiview.rusverbperfective.clozeInputHandler);
		$('body').undelegate('span.wertiviewhint', 'click', wertiview.rusverbperfective.clozeHintHandler);
		$('body').undelegate('input.wertiviewinput', 'change', wertiview.rusverbperfective.clozeInputHandler);
		$('body').undelegate('input.wertiviewhint', 'click', wertiview.rusverbperfective.clozeHintHandler);  // was: span.wertiviewhint
...
span.find('span.wertiviewhit').addClass('colorizeStyleRusVerbPerfective');
	...
	click: function(contextDoc) {
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.rusverbperfective.clickHandler); 
	},
	...
	mcGetOptions: function($hit, capType){
        ...
        while (j < wertiview.rusverbperfective.types.length && options.length < wertiview.rusverbperfective.MAX_MC - 1) {
            ...
            if (wertiview.rusverbperfective.types[j].toLowerCase() != $hit.text().toLowerCase() && wertiview.rusverbperfective.types[j] != "") 
            {
