wertiview.ns(function() {
	...
	remove: function(contextDoc) {
		...
		$('body').undelegate('span.wertiviewtoken', 'click', wertiview.rusadjectivemasculine.clickHandler);
		$('body').undelegate('select.wertiviewinput', 'change', wertiview.rusadjectivemasculine.clozeInputHandler);
		$('body').undelegate('span.wertiviewhint', 'click', wertiview.rusadjectivemasculine.clozeHintHandler);
		$('body').undelegate('input.wertiviewinput', 'change', wertiview.rusadjectivemasculine.clozeInputHandler);
		$('body').undelegate('input.wertiviewhint', 'click', wertiview.rusadjectivemasculine.clozeHintHandler);  // was: span.wertiviewhint
...
span.find('span.wertiviewhit').addClass('colorizeStyleRusAdjectiveMasculine');
	...
	click: function(contextDoc) {
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.rusadjectivemasculine.clickHandler); 
	},
	...
	mcGetOptions: function($hit, capType){
        ...
        while (j < wertiview.rusadjectivemasculine.types.length && options.length < wertiview.rusadjectivemasculine.MAX_MC - 1) {
            ...
            if (wertiview.rusadjectivemasculine.types[j].toLowerCase() != $hit.text().toLowerCase() && wertiview.rusadjectivemasculine.types[j] != "") 
            {
