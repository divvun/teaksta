  ...
  // maximum number of instances to turn into exercises (moved to preferences)
					...
					if (spanTags[i].hasChildNodes()) {
						...
						for (var k = 0; k < children.length; k++) {
							if (children[k].classList) {
								if (children[k].classList.contains('wertiviewAdverbial')) {
									children[k].classList.add('colorizeStyleAdverbial');
								}
							}
						}
					}
	...
	clickHandler: function(event) {
		...
		if (!widget_found && !menu_found && !a_found) {
			if (this.classList.contains('wertiviewAdverbial')) {  // was: wertiviewhit
	...
	mc: function(contextDoc) {
		...
		var spanTags = $('span.wertiviewAdverbial');
