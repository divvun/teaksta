	...
	remove: function(contextDoc) {
		...
		$('body').undelegate('span.wertiviewtoken', 'click', wertiview.presfutind.clickHandler);
		$('body').undelegate('select.wertiviewinput', 'change', wertiview.presfutind.clozeInputHandler);
		$('body').undelegate('span.wertiviewhint', 'click', wertiview.presfutind.clozeHintHandler);
		$('body').undelegate('input.wertiviewinput', 'change', wertiview.presfutind.clozeInputHandler);
		$('body').undelegate('input.wertiviewhint', 'click', wertiview.presfutind.clozeHintHandler);  // was: span.wertiviewhint
	...
	colorize: function(contextDoc) {
		...
		$('span.wertiviewPresFutInd').addClass('colorizeStylePresFutInd');
	},
	
	colorizeSpan: function(span, topic) {
span.find('span.wertiviewPresFutInd').addClass('colorizeStylePresFutInd');
	},
	...
	click: function(contextDoc) {
		...
		// conjunction markup
		$('span.wertiviewRELEVANT').find('span.wertiviewPresFutInd').addClass('colorizeStylePresFutInd');
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.presfutind.clickHandler); 
	},
	...
	clickHandler: function(event) {
		...
		if($(this).hasClass('wertiviewPresFutInd')) {  // was: wertiviewhit
	...
	mc: function(contextDoc) {
		...
		// get potential spans
		var $hits = $('span.wertiviewPresFutInd');
		
		//var hitList = [];
		var tokens = [];
		wertiview.presfutind.types = [];
		wertiview.presfutind.hitList = [];
		//alert($hits.length+" hits");
		$hits.each( function() {
			wertiview.presfutind.hitList.push($(this));
			//alert($(this).text());
			tokens[$(this).text().toLowerCase()] = 1;
		});
		//alert("number of tokens: "+tokens.length);
		//alert("size of hitList: "+wertiview.presfutind.hitList.length);
		/*for (word in tokens) {
			wertiview.presfutind.types.push(word);
			//alert("word: "+word);
		}*/
		//alert(wertiview.presfutind.types.length+" different presfutind on page");

		wertiview.presfutind.maxLength = wertiview.presfutind.MAX_MC;
		/*if (wertiview.presfutind.maxLength > wertiview.presfutind.types.length) {
			wertiview.presfutind.maxLength = wertiview.presfutind.types.length;
		}*/
		...
		wertiview.activity.mc(contextDoc, wertiview.presfutind.hitList, 
				wertiview.presfutind.clozeInputHandler, 
				wertiview.presfutind.clozeHintHandler, 
				wertiview.presfutind.mcGetOptions, 
				wertiview.presfutind.mcGetCorrectAnswer);
	...
	mcGetOptions: function($hit, capType){
		...
		wertiview.presfutind.types = $hit.attr('distractors').split(" ");
	    wertiview.lib.shuffleList(wertiview.presfutind.types);
        
        // Add the distractor forms to the options list:
        while (j < wertiview.presfutind.types.length && options.length < wertiview.presfutind.MAX_MC - 1) {
            // The forms that are homonymous to the correct form are excluded from the list of options:
            if (wertiview.presfutind.types[j] != $hit.text().toLowerCase() && wertiview.presfutind.types[j] != "") 
            {
                var homonym = false;
                var k = 0;
                while (k < j) //check for homonymes among the distractors
                { 
                    if (wertiview.presfutind.types[k] == wertiview.presfutind.types[j])
                        homonym = true;
                    k++;
                } 
                if (!homonym)                 options.push(wertiview.lib.matchCapitalization(wertiview.presfutind.types[j], capType)); 
            }
			j++;
		}
	...
	cloze: function(contextDoc) {
		...
		wertiview.activity.cloze(contextDoc, hitList, 
				...
				wertiview.presfutind.clozeHintHandler, 
				wertiview.presfutind.mcGetCorrectAnswer,
				wertiview.presfutind.clozeAddBaseform);
	},
