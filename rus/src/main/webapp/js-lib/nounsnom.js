	...
	remove: function(contextDoc) {
		...
		$('body').undelegate('span.wertiviewtoken', 'click', wertiview.nounsnom.clickHandler);
		$('body').undelegate('select.wertiviewinput', 'change', wertiview.nounsnom.clozeInputHandler);
		$('body').undelegate('span.wertiviewhint', 'click', wertiview.nounsnom.clozeHintHandler);
		$('body').undelegate('input.wertiviewinput', 'change', wertiview.nounsnom.clozeInputHandler);
		$('body').undelegate('input.wertiviewhint', 'click', wertiview.nounsnom.clozeHintHandler);  // was: span.wertiviewhint
	...
	colorize: function(contextDoc) {
		...
		$('span.wertiviewNounsNom').addClass('colorizeStyleNounsNom');
	},
	
	colorizeSpan: function(span, topic) {
span.find('span.wertiviewNounsNom').addClass('colorizeStyleNounsNom');
	},
	...
	click: function(contextDoc) {
		...
		// conjunction markup
		$('span.wertiviewRELEVANT').find('span.wertiviewNounsNom').addClass('colorizeStyleNounsNom');
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.nounsnom.clickHandler); 
	},
	...
	clickHandler: function(event) {
		...
		if($(this).hasClass('wertiviewNounsNom')) {  // was: wertiviewhit
	...
	mc: function(contextDoc) {
		...
		// get potential spans
		var $hits = $('span.wertiviewNounsNom');
		
		//var hitList = [];
		var tokens = [];
		wertiview.nounsnom.types = [];
		wertiview.nounsnom.hitList = [];
		//alert($hits.length+" hits");
		$hits.each( function() {
			wertiview.nounsnom.hitList.push($(this));
			//alert($(this).text());
			tokens[$(this).text().toLowerCase()] = 1;
		});
		//alert("number of tokens: "+tokens.length);
		//alert("size of hitList: "+wertiview.nounsnom.hitList.length);
		/*for (word in tokens) {
			wertiview.nounsnom.types.push(word);
			//alert("word: "+word);
		}*/
		//alert(wertiview.nounsnom.types.length+" different nounsnom on page");

		wertiview.nounsnom.maxLength = wertiview.nounsnom.MAX_MC;
		/*if (wertiview.nounsnom.maxLength > wertiview.nounsnom.types.length) {
			wertiview.nounsnom.maxLength = wertiview.nounsnom.types.length;
		}*/
		...
		wertiview.activity.mc(contextDoc, wertiview.nounsnom.hitList, 
				wertiview.nounsnom.clozeInputHandler, 
				wertiview.nounsnom.clozeHintHandler, 
				wertiview.nounsnom.mcGetOptions, 
				wertiview.nounsnom.mcGetCorrectAnswer);
	...
	mcGetOptions: function($hit, capType){
		...
		wertiview.nounsnom.types = $hit.attr('distractors').split(" ");
	    wertiview.lib.shuffleList(wertiview.nounsnom.types);
        
        // Add the distractor forms to the options list:
        while (j < wertiview.nounsnom.types.length && options.length < wertiview.nounsnom.MAX_MC - 1) {
            // The forms that are homonymous to the correct form are excluded from the list of options:
            if (wertiview.nounsnom.types[j] != $hit.text().toLowerCase() && wertiview.nounsnom.types[j] != "") 
            {
                var homonym = false;
                var k = 0;
                while (k < j) //check for homonymes among the distractors
                { 
                    if (wertiview.nounsnom.types[k] == wertiview.nounsnom.types[j])
                        homonym = true;
                    k++;
                } 
                if (!homonym)                 options.push(wertiview.lib.matchCapitalization(wertiview.nounsnom.types[j], capType)); 
            }
			j++;
		}
	...
	cloze: function(contextDoc) {
		...
		wertiview.activity.cloze(contextDoc, hitList, 
				...
				wertiview.nounsnom.clozeHintHandler, 
				wertiview.nounsnom.mcGetCorrectAnswer,
				wertiview.nounsnom.clozeAddBaseform);
	},
