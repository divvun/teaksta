	...
	remove: function(contextDoc) {
		...
		$('body').undelegate('span.wertiviewtoken', 'click', wertiview.nounsins.clickHandler);
		$('body').undelegate('select.wertiviewinput', 'change', wertiview.nounsins.clozeInputHandler);
		$('body').undelegate('span.wertiviewhint', 'click', wertiview.nounsins.clozeHintHandler);
		$('body').undelegate('input.wertiviewinput', 'change', wertiview.nounsins.clozeInputHandler);
		$('body').undelegate('input.wertiviewhint', 'click', wertiview.nounsins.clozeHintHandler);  // was: span.wertiviewhint
	...
	colorize: function(contextDoc) {
		...
		$('span.wertiviewNounsIns').addClass('colorizeStyleNounsIns');
	},
	
	colorizeSpan: function(span, topic) {
span.find('span.wertiviewNounsIns').addClass('colorizeStyleNounsIns');
	},
	...
	click: function(contextDoc) {
		...
		// conjunction markup
		$('span.wertiviewRELEVANT').find('span.wertiviewNounsIns').addClass('colorizeStyleNounsIns');
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.nounsins.clickHandler); 
	},
	...
	clickHandler: function(event) {
		...
		if($(this).hasClass('wertiviewNounsIns')) {  // was: wertiviewhit
	...
	mc: function(contextDoc) {
		...
		// get potential spans
		var $hits = $('span.wertiviewNounsIns');
		
		//var hitList = [];
		var tokens = [];
		wertiview.nounsins.types = [];
		wertiview.nounsins.hitList = [];
		//alert($hits.length+" hits");
		$hits.each( function() {
			wertiview.nounsins.hitList.push($(this));
			//alert($(this).text());
			tokens[$(this).text().toLowerCase()] = 1;
		});
		//alert("number of tokens: "+tokens.length);
		//alert("size of hitList: "+wertiview.nounsins.hitList.length);
		/*for (word in tokens) {
			wertiview.nounsins.types.push(word);
			//alert("word: "+word);
		}*/
		//alert(wertiview.nounsins.types.length+" different nounsins on page");

		wertiview.nounsins.maxLength = wertiview.nounsins.MAX_MC;
		/*if (wertiview.nounsins.maxLength > wertiview.nounsins.types.length) {
			wertiview.nounsins.maxLength = wertiview.nounsins.types.length;
		}*/
		...
		wertiview.activity.mc(contextDoc, wertiview.nounsins.hitList, 
				wertiview.nounsins.clozeInputHandler, 
				wertiview.nounsins.clozeHintHandler, 
				wertiview.nounsins.mcGetOptions, 
				wertiview.nounsins.mcGetCorrectAnswer);
	...
	mcGetOptions: function($hit, capType){
		...
		wertiview.nounsins.types = $hit.attr('distractors').split(" ");
	    wertiview.lib.shuffleList(wertiview.nounsins.types);
        
        // Add the distractor forms to the options list:
        while (j < wertiview.nounsins.types.length && options.length < wertiview.nounsins.MAX_MC - 1) {
            // The forms that are homonymous to the correct form are excluded from the list of options:
            if (wertiview.nounsins.types[j] != $hit.text().toLowerCase() && wertiview.nounsins.types[j] != "") 
            {
                var homonym = false;
                var k = 0;
                while (k < j) //check for homonymes among the distractors
                { 
                    if (wertiview.nounsins.types[k] == wertiview.nounsins.types[j])
                        homonym = true;
                    k++;
                } 
                if (!homonym)                 options.push(wertiview.lib.matchCapitalization(wertiview.nounsins.types[j], capType)); 
            }
			j++;
		}
	...
	cloze: function(contextDoc) {
		...
		wertiview.activity.cloze(contextDoc, hitList, 
				...
				wertiview.nounsins.clozeHintHandler, 
				wertiview.nounsins.mcGetCorrectAnswer,
				wertiview.nounsins.clozeAddBaseform);
	},
