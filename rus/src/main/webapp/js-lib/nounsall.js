	...
	remove: function(contextDoc) {
		...
		$('body').undelegate('span.wertiviewtoken', 'click', wertiview.nounsall.clickHandler);
		$('body').undelegate('select.wertiviewinput', 'change', wertiview.nounsall.clozeInputHandler);
		$('body').undelegate('span.wertiviewhint', 'click', wertiview.nounsall.clozeHintHandler);
		$('body').undelegate('input.wertiviewinput', 'change', wertiview.nounsall.clozeInputHandler);
		$('body').undelegate('input.wertiviewhint', 'click', wertiview.nounsall.clozeHintHandler);  // was: span.wertiviewhint
	...
	colorize: function(contextDoc) {
		...
		$('span.wertiviewNounsAll').addClass('colorizeStyleNounsAll');
	},
	
	colorizeSpan: function(span, topic) {
span.find('span.wertiviewNounsAll').addClass('colorizeStyleNounsAll');
	},
	...
	click: function(contextDoc) {
		...
		// conjunction markup
		$('span.wertiviewRELEVANT').find('span.wertiviewNounsAll').addClass('colorizeStyleNounsAll');
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.nounsall.clickHandler); 
	},
	...
	clickHandler: function(event) {
		...
		if($(this).hasClass('wertiviewNounsAll')) {  // was: wertiviewhit
	...
	mc: function(contextDoc) {
		...
		// get potential spans
		var $hits = $('span.wertiviewNounsAll');
		
		//var hitList = [];
		var tokens = [];
		wertiview.nounsall.types = [];
		wertiview.nounsall.hitList = [];
		//alert($hits.length+" hits");
		$hits.each( function() {
			wertiview.nounsall.hitList.push($(this));
			//alert($(this).text());
			tokens[$(this).text().toLowerCase()] = 1;
		});
		//alert("number of tokens: "+tokens.length);
		//alert("size of hitList: "+wertiview.nounsall.hitList.length);
		/*for (word in tokens) {
			wertiview.nounsall.types.push(word);
			//alert("word: "+word);
		}*/
		//alert(wertiview.nounsall.types.length+" different nounsall on page");

		wertiview.nounsall.maxLength = wertiview.nounsall.MAX_MC;
		/*if (wertiview.nounsall.maxLength > wertiview.nounsall.types.length) {
			wertiview.nounsall.maxLength = wertiview.nounsall.types.length;
		}*/
		...
		wertiview.activity.mc(contextDoc, wertiview.nounsall.hitList, 
				wertiview.nounsall.clozeInputHandler, 
				wertiview.nounsall.clozeHintHandler, 
				wertiview.nounsall.mcGetOptions, 
				wertiview.nounsall.mcGetCorrectAnswer);
	...
	mcGetOptions: function($hit, capType){
		...
		wertiview.nounsall.types = $hit.attr('distractors').split(" ");
	    wertiview.lib.shuffleList(wertiview.nounsall.types);
        
        // Add the distractor forms to the options list:
        while (j < wertiview.nounsall.types.length && options.length < wertiview.nounsall.MAX_MC - 1) {
            // The forms that are homonymous to the correct form are excluded from the list of options:
            if (wertiview.nounsall.types[j] != $hit.text().toLowerCase() && wertiview.nounsall.types[j] != "") 
            {
                var homonym = false;
                var k = 0;
                while (k < j) //check for homonymes among the distractors
                { 
                    if (wertiview.nounsall.types[k] == wertiview.nounsall.types[j])
                        homonym = true;
                    k++;
                } 
                if (!homonym)                 options.push(wertiview.lib.matchCapitalization(wertiview.nounsall.types[j], capType)); 
            }
			j++;
		}
	...
	cloze: function(contextDoc) {
		...
		wertiview.activity.cloze(contextDoc, hitList, 
				...
				wertiview.nounsall.clozeHintHandler, 
				wertiview.nounsall.mcGetCorrectAnswer,
				wertiview.nounsall.clozeAddBaseform);
	},
