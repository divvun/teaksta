 ...
 *
 */
public class Vislcg3NounsDatEnhancer extends JCasAnnotator_ImplBase {

	private static final Logger log =
		Logger.getLogger(Vislcg3NounsDatEnhancer.class);
	...
	*/
	...
	public void initialize(UimaContext context)
        ...
        log.info("Dat Noun tags "+NDatTags);
		super.initialize(context);
		NDatTags = Arrays.asList(((String)context.getConfigParameterValue("NDatTags")).split(","));
	}

	@Override
	public void process(JCas cas) throws AnalysisEngineProcessException {
		log.info("Starting Dat Noun enhancement");
		...
		for (String conT: NDatTags) {
			...
			while (cgTokenIter.hasNext()) {
				...
				if (containsTag(reading, conT, enhancement_type)) {
					...
					String spanStartTag = "<span id=\"" + EnhancerUtils.get_id("WERTi-span-" + conT, newId) + "\" class=\"wertiviewtoken  wertiviewNounsDat \" lemma=\"" + lemma + "\" distractors=\"" + distractors + "\">";
		...
		log.info("Finished Dat Noun enhancement");
	}
