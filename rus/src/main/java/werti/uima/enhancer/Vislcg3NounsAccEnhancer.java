 ...
 *
 */
public class Vislcg3NounsAccEnhancer extends JCasAnnotator_ImplBase {

	private static final Logger log =
		Logger.getLogger(Vislcg3NounsAccEnhancer.class);
	...
	*/
	...
	public void initialize(UimaContext context)
        ...
        log.info("Acc Noun tags "+NAccTags);
		super.initialize(context);
		NAccTags = Arrays.asList(((String)context.getConfigParameterValue("NAccTags")).split(","));
	}

	@Override
	public void process(JCas cas) throws AnalysisEngineProcessException {
		log.info("Starting Acc Noun enhancement");
		...
		for (String conT: NAccTags) {
			...
			while (cgTokenIter.hasNext()) {
				...
				if (containsTag(reading, conT, enhancement_type)) {
					...
					String spanStartTag = "<span id=\"" + EnhancerUtils.get_id("WERTi-span-" + conT, newId) + "\" class=\"wertiviewtoken  wertiviewNounsAcc \" lemma=\"" + lemma + "\" distractors=\"" + distractors + "\">";
		...
		log.info("Finished Acc Noun enhancement");
	}
