 ...
 *
 */
public class Vislcg3NounsInsEnhancer extends JCasAnnotator_ImplBase {

	private static final Logger log =
		Logger.getLogger(Vislcg3NounsInsEnhancer.class);
	...
	*/
	...
	public void initialize(UimaContext context)
        ...
        log.info("Ins Noun tags "+NInsTags);
		super.initialize(context);
		NInsTags = Arrays.asList(((String)context.getConfigParameterValue("NInsTags")).split(","));
	}

	@Override
	public void process(JCas cas) throws AnalysisEngineProcessException {
		log.info("Starting Ins Noun enhancement");
		...
		for (String conT: NInsTags) {
			...
			while (cgTokenIter.hasNext()) {
				...
				if (containsTag(reading, conT, enhancement_type)) {
					...
					String spanStartTag = "<span id=\"" + EnhancerUtils.get_id("WERTi-span-" + conT, newId) + "\" class=\"wertiviewtoken  wertiviewNounsIns \" lemma=\"" + lemma + "\" distractors=\"" + distractors + "\">";
		...
		log.info("Finished Ins Noun enhancement");
	}
