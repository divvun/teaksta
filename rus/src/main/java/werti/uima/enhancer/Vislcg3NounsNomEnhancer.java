 ...
 *
 */
public class Vislcg3NounsNomEnhancer extends JCasAnnotator_ImplBase {

	private static final Logger log =
		Logger.getLogger(Vislcg3NounsNomEnhancer.class);
	...
	*/
	...
	public void initialize(UimaContext context)
        ...
        log.info("Nom Noun tags "+NNomTags);
		super.initialize(context);
		NNomTags = Arrays.asList(((String)context.getConfigParameterValue("NNomTags")).split(","));
	}

	@Override
	public void process(JCas cas) throws AnalysisEngineProcessException {
		log.info("Starting Nom Noun enhancement");
		...
		for (String conT: NNomTags) {
			...
			while (cgTokenIter.hasNext()) {
				...
				if (containsTag(reading, conT, enhancement_type)) {
					...
					String spanStartTag = "<span id=\"" + EnhancerUtils.get_id("WERTi-span-" + conT, newId) + "\" class=\"wertiviewtoken  wertiviewNounsNom \" lemma=\"" + lemma + "\" distractors=\"" + distractors + "\">";
		...
		log.info("Finished Nom Noun enhancement");
	}
