	...
	remove: function(contextDoc) {
		...
		$('body').undelegate('span.wertiviewtoken', 'click', wertiview.nounsacc.clickHandler);
		$('body').undelegate('select.wertiviewinput', 'change', wertiview.nounsacc.clozeInputHandler);
		$('body').undelegate('span.wertiviewhint', 'click', wertiview.nounsacc.clozeHintHandler);
		$('body').undelegate('input.wertiviewinput', 'change', wertiview.nounsacc.clozeInputHandler);
		$('body').undelegate('input.wertiviewhint', 'click', wertiview.nounsacc.clozeHintHandler);  // was: span.wertiviewhint
	...
	colorize: function(contextDoc) {
		...
		$('span.wertiviewNounsAcc').addClass('colorizeStyleNounsAcc');
	},
	
	colorizeSpan: function(span, topic) {
span.find('span.wertiviewNounsAcc').addClass('colorizeStyleNounsAcc');
	},
	...
	click: function(contextDoc) {
		...
		// conjunction markup
		$('span.wertiviewRELEVANT').find('span.wertiviewNounsAcc').addClass('colorizeStyleNounsAcc');
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.nounsacc.clickHandler); 
	},
	...
	clickHandler: function(event) {
		...
		if($(this).hasClass('wertiviewNounsAcc')) {  // was: wertiviewhit
	...
	mc: function(contextDoc) {
		...
		// get potential spans
		var $hits = $('span.wertiviewNounsAcc');
		
		//var hitList = [];
		var tokens = [];
		wertiview.nounsacc.types = [];
		wertiview.nounsacc.hitList = [];
		//alert($hits.length+" hits");
		$hits.each( function() {
			wertiview.nounsacc.hitList.push($(this));
			//alert($(this).text());
			tokens[$(this).text().toLowerCase()] = 1;
		});
		//alert("number of tokens: "+tokens.length);
		//alert("size of hitList: "+wertiview.nounsacc.hitList.length);
		/*for (word in tokens) {
			wertiview.nounsacc.types.push(word);
			//alert("word: "+word);
		}*/
		//alert(wertiview.nounsacc.types.length+" different nounsacc on page");

		wertiview.nounsacc.maxLength = wertiview.nounsacc.MAX_MC;
		/*if (wertiview.nounsacc.maxLength > wertiview.nounsacc.types.length) {
			wertiview.nounsacc.maxLength = wertiview.nounsacc.types.length;
		}*/
		...
		wertiview.activity.mc(contextDoc, wertiview.nounsacc.hitList, 
				wertiview.nounsacc.clozeInputHandler, 
				wertiview.nounsacc.clozeHintHandler, 
				wertiview.nounsacc.mcGetOptions, 
				wertiview.nounsacc.mcGetCorrectAnswer);
	...
	mcGetOptions: function($hit, capType){
		...
		wertiview.nounsacc.types = $hit.attr('distractors').split(" ");
	    wertiview.lib.shuffleList(wertiview.nounsacc.types);
        
        // Add the distractor forms to the options list:
        while (j < wertiview.nounsacc.types.length && options.length < wertiview.nounsacc.MAX_MC - 1) {
            // The forms that are homonymous to the correct form are excluded from the list of options:
            if (wertiview.nounsacc.types[j] != $hit.text().toLowerCase() && wertiview.nounsacc.types[j] != "") 
            {
                var homonym = false;
                var k = 0;
                while (k < j) //check for homonymes among the distractors
                { 
                    if (wertiview.nounsacc.types[k] == wertiview.nounsacc.types[j])
                        homonym = true;
                    k++;
                } 
                if (!homonym)                 options.push(wertiview.lib.matchCapitalization(wertiview.nounsacc.types[j], capType)); 
            }
			j++;
		}
	...
	cloze: function(contextDoc) {
		...
		wertiview.activity.cloze(contextDoc, hitList, 
				...
				wertiview.nounsacc.clozeHintHandler, 
				wertiview.nounsacc.mcGetCorrectAnswer,
				wertiview.nounsacc.clozeAddBaseform);
	},
