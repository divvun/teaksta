	...
	remove: function(contextDoc) {
		...
		$('body').undelegate('span.wertiviewtoken', 'click', wertiview.adverbial.clickHandler);
		$('body').undelegate('select.wertiviewinput', 'change', wertiview.adverbial.clozeInputHandler);
		$('body').undelegate('span.wertiviewhint', 'click', wertiview.adverbial.clozeHintHandler);
		$('body').undelegate('input.wertiviewinput', 'change', wertiview.adverbial.clozeInputHandler);
		$('body').undelegate('input.wertiviewhint', 'click', wertiview.adverbial.clozeHintHandler);  // was: span.wertiviewhint
	...
	colorize: function(contextDoc) {
		...
		$('span.wertiviewAdverbial').addClass('colorizeStyleAdverbial');
	},
	
	colorizeSpan: function(span, topic) {
span.find('span.wertiviewAdverbial').addClass('colorizeStyleAdverbial');
	},
	...
	click: function(contextDoc) {
		...
		// adverbial markup
		$('span.wertiviewRELEVANT').find('span.wertiviewAdverbial').addClass('colorizeStyleAdverbial');
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.adverbial.clickHandler); 
	},
	...
	clickHandler: function(event) {
		...
		if($(this).hasClass('wertiviewAdverbial')) {  // was: wertiviewhit
	...
	mc: function(contextDoc) {
		...
		// get potential spans
		var $hits = $('span.wertiviewAdverbial');
		
		//var hitList = [];
		var tokens = [];
		wertiview.adverbial.types = [];
		wertiview.adverbial.hitList = [];
		//alert($hits.length+" hits");
		$hits.each( function() {
			wertiview.adverbial.hitList.push($(this));
			//alert($(this).text());
			tokens[$(this).text().toLowerCase()] = 1;
		});
		//alert("number of tokens: "+tokens.length);
		//alert("size of hitList: "+wertiview.adverbial.hitList.length);
		/*for (word in tokens) {
			wertiview.adverbial.types.push(word);
			//alert("word: "+word);
		}*/
		//alert(wertiview.adverbial.types.length+" different adverbial on page");

		wertiview.adverbial.maxLength = wertiview.adverbial.MAX_MC;
		/*if (wertiview.adverbial.maxLength > wertiview.adverbial.types.length) {
			wertiview.adverbial.maxLength = wertiview.adverbial.types.length;
		}*/
		...
		wertiview.activity.mc(contextDoc, wertiview.adverbial.hitList, 
				wertiview.adverbial.clozeInputHandler, 
				wertiview.adverbial.clozeHintHandler, 
				wertiview.adverbial.mcGetOptions, 
				wertiview.adverbial.mcGetCorrectAnswer);
	...
	mcGetOptions: function($hit, capType){
		...
		wertiview.adverbial.types = $hit.attr('distractors').split(" ");
	    wertiview.lib.shuffleList(wertiview.adverbial.types);
        
        // Add the distractor forms to the options list:
        while (j < wertiview.adverbial.types.length && options.length < wertiview.adverbial.MAX_MC - 1) {
            // The forms that are homonymous to the correct form are excluded from the list of options:
            if (wertiview.adverbial.types[j] != $hit.text().toLowerCase() && wertiview.adverbial.types[j] != "") 
            {
                var homonym = false;
                var k = 0;
                while (k < j) //check for homonymes among the distractors
                { 
                    if (wertiview.adverbial.types[k] == wertiview.adverbial.types[j])
                        homonym = true;
                    k++;
                } 
                if (!homonym)                 options.push(wertiview.lib.matchCapitalization(wertiview.adverbial.types[j], capType)); 
            }
			j++;
		}
	...
	cloze: function(contextDoc) {
		...
		wertiview.activity.cloze(contextDoc, hitList, 
				...
				wertiview.adverbial.clozeHintHandler, 
				wertiview.adverbial.mcGetCorrectAnswer,
				wertiview.adverbial.clozeAddBaseform);
	},
