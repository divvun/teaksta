	...
	remove: function(contextDoc) {
		...
		$('body').undelegate('span.wertiviewtoken', 'click', wertiview.nounsgen.clickHandler);
		$('body').undelegate('select.wertiviewinput', 'change', wertiview.nounsgen.clozeInputHandler);
		$('body').undelegate('span.wertiviewhint', 'click', wertiview.nounsgen.clozeHintHandler);
		$('body').undelegate('input.wertiviewinput', 'change', wertiview.nounsgen.clozeInputHandler);
		$('body').undelegate('input.wertiviewhint', 'click', wertiview.nounsgen.clozeHintHandler);  // was: span.wertiviewhint
	...
	colorize: function(contextDoc) {
		...
		$('span.wertiviewNounsGen').addClass('colorizeStyleNounsGen');
	},
	
	colorizeSpan: function(span, topic) {
span.find('span.wertiviewNounsGen').addClass('colorizeStyleNounsGen');
	},
	...
	click: function(contextDoc) {
		...
		// conjunction markup
		$('span.wertiviewRELEVANT').find('span.wertiviewNounsGen').addClass('colorizeStyleNounsGen');
		...
		// handle click
		$('body').delegate('span.wertiviewtoken', 'click', {context: contextDoc}, wertiview.nounsgen.clickHandler); 
	},
	...
	clickHandler: function(event) {
		...
		if($(this).hasClass('wertiviewNounsGen')) {  // was: wertiviewhit
	...
	mc: function(contextDoc) {
		...
		// get potential spans
		var $hits = $('span.wertiviewNounsGen');
		
		//var hitList = [];
		var tokens = [];
		wertiview.nounsgen.types = [];
		wertiview.nounsgen.hitList = [];
		//alert($hits.length+" hits");
		$hits.each( function() {
			wertiview.nounsgen.hitList.push($(this));
			//alert($(this).text());
			tokens[$(this).text().toLowerCase()] = 1;
		});
		//alert("number of tokens: "+tokens.length);
		//alert("size of hitList: "+wertiview.nounsgen.hitList.length);
		/*for (word in tokens) {
			wertiview.nounsgen.types.push(word);
			//alert("word: "+word);
		}*/
		//alert(wertiview.nounsgen.types.length+" different nounsgen on page");

		wertiview.nounsgen.maxLength = wertiview.nounsgen.MAX_MC;
		/*if (wertiview.nounsgen.maxLength > wertiview.nounsgen.types.length) {
			wertiview.nounsgen.maxLength = wertiview.nounsgen.types.length;
		}*/
		...
		wertiview.activity.mc(contextDoc, wertiview.nounsgen.hitList, 
				wertiview.nounsgen.clozeInputHandler, 
				wertiview.nounsgen.clozeHintHandler, 
				wertiview.nounsgen.mcGetOptions, 
				wertiview.nounsgen.mcGetCorrectAnswer);
	...
	mcGetOptions: function($hit, capType){
		...
		wertiview.nounsgen.types = $hit.attr('distractors').split(" ");
	    wertiview.lib.shuffleList(wertiview.nounsgen.types);
        
        // Add the distractor forms to the options list:
        while (j < wertiview.nounsgen.types.length && options.length < wertiview.nounsgen.MAX_MC - 1) {
            // The forms that are homonymous to the correct form are excluded from the list of options:
            if (wertiview.nounsgen.types[j] != $hit.text().toLowerCase() && wertiview.nounsgen.types[j] != "") 
            {
                var homonym = false;
                var k = 0;
                while (k < j) //check for homonymes among the distractors
                { 
                    if (wertiview.nounsgen.types[k] == wertiview.nounsgen.types[j])
                        homonym = true;
                    k++;
                } 
                if (!homonym)                 options.push(wertiview.lib.matchCapitalization(wertiview.nounsgen.types[j], capType)); 
            }
			j++;
		}
	...
	cloze: function(contextDoc) {
		...
		wertiview.activity.cloze(contextDoc, hitList, 
				...
				wertiview.nounsgen.clozeHintHandler, 
				wertiview.nounsgen.mcGetCorrectAnswer,
				wertiview.nounsgen.clozeAddBaseform);
	},
